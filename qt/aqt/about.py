...
def show(mw: aqt.AnkiQt) -> QDialog:
    ...
    with open("CONTRIBUTORS") as f:
        inside_name_section = False
        for line in f.readlines():
            if line.lstrip().startswith("*" * 10):
                inside_name_section = not inside_name_section
                continue
            if inside_name_section:
                match = re.match(r"(.+)\s*<", line)
                if match:
                    code_contributors.append(match.group(1).rstrip())
                elif line.strip():
                    code_contributors.append(line.rstrip())
    ...
    contributors = sorted(list(set(misc_contributors + code_contributors)))

    abouttext += "<p>" + tr.about_written_by_damien_elmes_with_patches(
        cont=", ".join(contributors)
    )
